...
(
                                            64
) NOT NULL COMMENT '表名称',
    `entity_name` varchar
(
    60
) NOT NULL COMMENT '实体名称',
    `list_type` int NOT NULL DEFAULT 1 COMMENT '列表类型（1 表格列表 2树状列表）',
    `tree_id` varchar
(
    60
) DEFAULT NULL COMMENT '树编码',
    `tree_pid` varchar
(
    60
) DEFAULT NULL COMMENT '树父编码',
    `tree_label` varchar
(
    60
) DEFAULT NULL COMMENT '树名称',
    `module_name` varchar
(
    60
...
(
    50
) NOT NULL COMMENT '业务名称',
    `package_name` varchar
(
    60
) NOT NULL COMMENT '包名称',
    `author` varchar
(
    100
) NOT NULL COMMENT '作者',
...
(
    20
) DEFAULT NULL COMMENT '表前缀',
    `is_override` bit
(
    1
) NOT NULL DEFAULT b'0' COMMENT '是否覆盖',
    `dialog_type` int DEFAULT NULL DEFAULT 1 COMMENT '弹窗类型（1 modal弹窗 2 drawer抽屉）',
    `parent_menu_id` bigint
(
    20
) DEFAULT 0 COMMENT '父级菜单id',
    `front_path` varchar
(
    255
) DEFAULT NULL COMMENT '前端项目路径',
