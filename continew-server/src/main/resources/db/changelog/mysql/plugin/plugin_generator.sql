...
CREATE TABLE IF NOT EXISTS `gen_config` (
    ...
    `entity_name`    VARCHAR(60)   NOT NULL                    COMMENT '实体名称',
    `list_type`      INT           NOT NULL   DEFAULT 1        COMMENT '列表类型（1 表格列表 2树状列表）',
    `tree_id`        VARCHAR(60)   DEFAULT NULL                COMMENT '树编码',
    `tree_pid`       VARCHAR(60)   DEFAULT NULL                COMMENT '树父编码',
    `tree_label`     VARCHAR(60)   DEFAULT NULL                COMMENT '树名称',
    ...
    `dialog_type`    INT           DEFAULT 1                   COMMENT '弹窗类型（1 modal弹窗 2 drawer抽屉）',
    `parent_menu_id` BIGINT(20)    DEFAULT 0                   COMMENT '父级菜单id',
    `front_path`     VARCHAR(255)  DEFAULT NULL                COMMENT '前端项目路径',
