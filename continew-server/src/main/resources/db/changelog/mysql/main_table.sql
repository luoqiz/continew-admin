...
CREATE TABLE IF NOT EXISTS `sys_role` (
    ...
    `data_scope`          tinyint      NOT NULL DEFAULT 4    COMMENT '数据权限（1：全部数据权限；2：本部门及以下数据权限；3：本部门数据权限；4：仅本人数据权限；5：自定义数据权限）',
    `description`         varchar(200) DEFAULT NULL          COMMENT '描述',
    `sort`                int          NOT NULL DEFAULT 999  COMMENT '排序',
    `is_system`           bit(1)       NOT NULL DEFAULT b'0' COMMENT '是否为系统内置数据',
...
CREATE TABLE IF NOT EXISTS `sys_log` (
    ...
    `time_taken`       bigint       NOT NULL                    COMMENT '耗时（ms）',
...
CREATE TABLE IF NOT EXISTS `sys_message` (
    ...
    `type`        tinyint      UNSIGNED NOT NULL DEFAULT 1 COMMENT '类型（1：系统消息；2：安全消息）',
    `path`        varchar(255) DEFAULT NULL                COMMENT '跳转路径',
    `scope`       tinyint      UNSIGNED NOT NULL DEFAULT 1 COMMENT '通知范围（1：所有人；2：指定用户）',
...
CREATE TABLE IF NOT EXISTS `sys_notice` (
    ...
    `notice_scope`   tinyint      UNSIGNED NOT NULL DEFAULT 1 COMMENT '通知范围（1：所有人；2：指定用户）',
    ...
    `status`         tinyint      UNSIGNED NOT NULL DEFAULT 1 COMMENT '状态（1：草稿；2：待发布；3：已发布）',
    `create_user`    bigint       NOT NULL                    COMMENT '创建人',
    `create_time`    datetime     NOT NULL                    COMMENT '创建时间',
    `update_user`    bigint       DEFAULT NULL                COMMENT '修改人',
    `update_time`    datetime     DEFAULT NULL                COMMENT '修改时间',
...
CREATE TABLE IF NOT EXISTS `sys_storage` (
    ...
    `code`                       varchar(30)  NOT NULL                    COMMENT '编码',
    `type`                       tinyint      UNSIGNED NOT NULL DEFAULT 1 COMMENT '类型（1：本地存储；2：对象存储）',
    ...
    `multipart_upload_threshold` bigint       DEFAULT NULL                COMMENT '分片上传阈值（字节）',
    `multipart_upload_part_size` bigint       DEFAULT NULL                COMMENT '分片上传大小（字节）',
    `multipart_temp_dir`         varchar(255) DEFAULT NULL                COMMENT '本地分片临时目录',
...
CREATE TABLE IF NOT EXISTS `sys_file` (
    ...
    `type`               tinyint      UNSIGNED NOT NULL DEFAULT 1 COMMENT '类型（0: 目录；1：其他；2：图片；3：文档；4：视频；5：音频）',
    ...
    `thumbnail_size`     bigint       DEFAULT NULL                COMMENT '缩略图大小（字节)',
...
CREATE TABLE IF NOT EXISTS `sys_client` (
    ...
    `active_timeout`       bigint      NOT NULL DEFAULT -1         COMMENT 'Token最低活跃频率（单位：秒，-1：不限制，永不冻结）',
    `timeout`              bigint      NOT NULL DEFAULT 2592000    COMMENT 'Token有效期（单位：秒，-1：永不过期）',
...
CREATE TABLE IF NOT EXISTS `sys_sms_log`  (
    ...
    `status`      tinyint     UNSIGNED NOT NULL DEFAULT 1 COMMENT '发送状态（1：成功；2：失败）',
