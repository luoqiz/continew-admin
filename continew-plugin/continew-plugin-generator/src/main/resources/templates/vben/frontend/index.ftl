...
import { delete${classNamePrefix}, export${classNamePrefix}, list${classNamePrefix} } from '#/api/${apiModuleName}/${apiName}'
import { useDownload } from '#/hooks/app/useDownload';

import { use${classNamePrefix}GridFieldColumns, use${classNamePrefix}GridSearchFormSchema } from './${classNamePrefix}Data';
<#if dialogType==1>
...
import ${classNamePrefix}InfoDrawer from './${classNamePrefix}InfoDrawer.vue';
<#if hasDictField>
...
const [InfoWindow, infoApi] = useVbenDrawer({
  connectedComponent: ${classNamePrefix}InfoDrawer,
  destroyOnClose: true,
});

const handleInfo = (record: ${classNamePrefix}Resp) => {
  infoApi.setData({ id: record.id });
  infoApi.open();
};
...
<template>
  <Page auto-content-height>
    <TableGrid :table-title="$t('${apiModuleName}.${apiName}.listTitle')">
      ...
      <template #${fieldConfig.fieldName}="{ row }">
        <DictTag :dict-list="${fieldConfig.dictCode}" :value="row.${fieldConfig.fieldName}" />
      </template>
      </#if>
      </#list>
      ...
      <template #action="{ row }">
        <ElSpace>
          <span v-access:code="['${apiModuleName}:${apiName}:get']">
            <ElButton type="primary" text link @click="handleInfo(row)">
              {{ $t('pages.common.info') }}
            </ElButton>
          </span>
          ...
          <span v-access:code="['${apiModuleName}:${apiName}:delete']">
            <ElPopconfirm :title="$t('ui.actionMessage.deleteConfirm', [row.name])" icon-color="red" @confirm="handleDelete(row)">
              <template #reference>
