...
public class AuthWebSocketSessionService implements AuthSessionRevocationNotifier {
    ...
    public void subscribe() {
        ...
        listenerId = revocationTopic.addListener(String.class,
            (channel, sessionId) -> this.closeLocal(sessionId));
    }
     ...
     */
    @Scheduled(fixedDelayString = "${auth.refresh-token.websocket-validation-interval:60000}")
    ...
    private void closeLocalAccessToken(WebSocketSessionDao sessionDao, String accessToken) {
        WebSocketSession webSocketSession = sessionDao.get(accessToken);
        try {
