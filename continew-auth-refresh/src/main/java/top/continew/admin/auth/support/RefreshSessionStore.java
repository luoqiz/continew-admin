...
public class RefreshSessionStore {
    ...
    /** 绑定 Access Token 指纹与所属 Session。 */
    public void bindAccessToken(String accessToken, String sessionId, long accessTokenTimeout,
        long sessionExpiresAt) {
        if (accessToken == null || accessToken.isBlank() || sessionId == null) {
            return;
        }
        String key = ACCESS_PREFIX + tokenCodec.fingerprint(accessToken);
        long sessionTimeout = this.remainingSeconds(sessionExpiresAt);
        long timeout = accessTokenTimeout == -1 ? sessionTimeout
            : Math.min(accessTokenTimeout, sessionTimeout);
        RedisUtils.set(key, sessionId, Duration.ofSeconds(Math.max(1, timeout)));
    }

    /** 查询 Access Token 所属的 Session ID。 */
    public String findSessionIdByAccessToken(String accessToken) {
        if (accessToken == null || accessToken.isBlank()) {
            return null;
        }
        return RedisUtils.get(ACCESS_PREFIX + tokenCodec.fingerprint(accessToken));
    }
    ...
    public AuthSecurityVersion getSecurityVersion(Long userId, String clientId, Long tenantId) {
        ...
        batch.execute();
        return new AuthSecurityVersion(userVersion.getNow(), clientVersion.getNow(),
            tenantVersion == null ? 0 : tenantVersion.getNow());
    }
