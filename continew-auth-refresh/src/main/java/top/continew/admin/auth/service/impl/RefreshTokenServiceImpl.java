...
public class RefreshTokenServiceImpl implements RefreshTokenService, AccessSessionValidator {
    ...
    private <T> void applyLoginPolicy(LoginAttempt<T> attempt) {
        ...
        String accessSessionId = sessionStore.findSessionIdByAccessToken(
            attempt.currentAccessToken());
        ...
        this.revokeSessions(replacedSessionIds);
    ...
    public String issue(String sessionId, UserContext userContext, RefreshClientPolicy client,
        ...
        try (RedisLockUtils ignored = sessionStore.lockSession(sessionId)) {
            ...
            sessionStore.bindAccessToken(accessToken, sessionId, accessTokenTimeout, expiresAt);
            if (!sessionStore.isSecurityVersionCurrent(session)) {
                this.revokeSessionLocked(session);
    ...
    private LoginResp completeRotationLocked(ParsedToken presentedToken, RefreshSession session,
        ...
        long now = System.currentTimeMillis();
        this.requireCurrentToken(session, newRefreshToken);
        sessionStore.bindAccessToken(loginResp.getAccessToken(), session.getSessionId(),
            loginResp.getExpiresIn(), session.getExpiresAt());
    ...
    public void revokeCurrent(String accessToken, String refreshToken) {
        Set<String> sessionIds = new LinkedHashSet<>();
        String accessSessionId = sessionStore.findSessionIdByAccessToken(accessToken);
        if (accessSessionId != null) {
    ...
    public boolean isInvalid(String accessToken) {
        ...
        String indexedSessionId = sessionStore.findSessionIdByAccessToken(accessToken);
        if (!Objects.equals(claimedSessionId, indexedSessionId)) {
            return true;
        }
    ...
    private void invalidateSessions(Supplier<AuthPolicyLock> policyLock,
        ...
        this.executeWithLocks(List.of(policyLock), () -> {
            ...
            this.revokeSessions(sessionIds.get());
            return null;
        });
    }
    ...
    private void revokeSessions(Set<String> sessionIds) {
    ...
    private void revokeSessionLocked(RefreshSession session) {
